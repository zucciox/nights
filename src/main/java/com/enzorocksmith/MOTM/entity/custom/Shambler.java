...
public class Shambler extends MoonMob implements GeoEntity {
    ...
    public Shambler(EntityType<? extends PathfinderMob> entityType, Level level) {
    ...
    private PlayState predicate(AnimationState<Shambler> animationState) {
    ...
    protected void tickDeath() {
        ...
        if (this.deathTime == 20) {
            this.remove(Shambler.RemovalReason.KILLED);
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putInt("triggerEffectTime", triggerEffectTime);
    }
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        if (tag.contains("triggerEffectTime")) {
            groupSize = tag.getInt("triggerEffectTime");
        }
    }
    ...
    public void meleeAttack() {
            ...
            if ((MOTMUtils.vecDist(this.getTarget().position(), this.position()) <= 1.75) && attackCoolDown <= 0 && !this.isDeadOrDying()) {
                attackCoolDown = 50 + random.nextInt(-10, 10);
                isAttacking = true;
                triggerAnim("attack", "attack");
                MOTM.queueServerWork(35, () -> {

                    if (this.getTarget() == null) return;
                    if ((MOTMUtils.vecDist(this.getTarget().position(), this.position()) <= 1.75) && this.hasLineOfSight(this.getTarget()))
                        this.getTarget().hurt(
    ...
    public BlockPos[] getBlocksInFront() {
        Vec3 eyePos = this.getEyePosition(1);
        ...
        BlockPos upper = this.level().clip(new ClipContext(eyePos, adjustedVec, ClipContext.Block.OUTLINE, net.minecraft.world.level.ClipContext.Fluid.NONE, this)).getBlockPos();
        BlockPos lower = BlockPos.containing(new Vec3(upper.getX(), upper.getY() - 1, upper.getZ()));

        if (heightDiff >= 1.5) {
            BlockPos head = BlockPos.containing(getEyePosition().x, getEyePosition().y+1, getEyePosition().z);
            BlockPos upperPlusOne = BlockPos.containing(new Vec3(upper.getX(), upper.getY() + 1, upper.getZ()));
            return new BlockPos[]{upper, upperPlusOne, head};
        }

        if (heightDiff <= -1.5) {
            BlockPos feet = BlockPos.containing(this.position().x, this.position().y-1, this.position().z);
            BlockPos lowerMinusOne = BlockPos.containing(new Vec3(lower.getX(), lower.getY() - 1, lower.getZ()));
            return new BlockPos[]{upper, lower, feet, lowerMinusOne};
        }

        return new BlockPos[]{upper, lower};
    }
    ...
    public void tryBlockBreak() {
        ...
        for (BlockPos block : blocks) {
            level.sendParticles(ParticleTypes.HAPPY_VILLAGER, block.getX() + 0, block.getY() + .5, block.getZ() + 0, 1, 0, 0, 0, 0);
        }
         */
        ...
        if (breakCoolDown <= 0 && (this.level().getBlockState(blocks[0]).isSolid())) {
    ...
    public int defineGroupSize() {
        List<Shambler> list = this.level().getEntitiesOfClass(Shambler.class, new AABB(this.position(), this.position()).inflate(10), e -> true);
        groupSize = list.size();
        return list.size();
    }

    public static boolean shamblerSpawnRules(EntityType<Shambler> entityType, LevelAccessor pLevel, MobSpawnType pSpawnType, BlockPos pPos, RandomSource pRandom) {
